...
session_start();
$token = $_GET['token'] ?? '';
$email = $_GET['email'] ?? ''; // Add email from URL parameters
$type = $_GET['type'] ?? '';

// Validate required parameters
if (empty($token) || empty($email) || empty($type)) {
    $_SESSION['reset_message'] = "Invalid password reset link. Missing required parameters.";
}
...
<body>
    <div class="container">
        ...
        <?php if (empty($token) || empty($email) || empty($type)): ?>
            <p class="error-message">Invalid password reset link. Please request a new password reset.</p>
        <?php else: ?>
